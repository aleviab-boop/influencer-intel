...
interface PhylloState {
  available: boolean;
  configured: boolean;
  env: string;
  connector: { status: string; username: string | null; connected_at: string | null } | null;
}
...
// Phyllo Connect Web SDK — loaded on demand only when a creator taps Connect (so
// nothing ships for the dormant/unconfigured case). Minimal surface we call.
interface PhylloConnectInstance {
  on(event: string, cb: (...args: unknown[]) => void): void;
  open(): void;
}
interface PhylloConnectSDK {
  initialize(config: {
...
async function loadPhylloSDK(): Promise<PhylloConnectSDK | null> {
  if (typeof window === 'undefined') return null;
  const w = window as unknown as { PhylloConnect?: PhylloConnectSDK };
  if (w.PhylloConnect) return w.PhylloConnect;
  await new Promise<void>((resolve, reject) => {
    const existing = document.querySelector(`script[src="${PHYLLO_SDK_URL}"]`);
    if (existing) { existing.addEventListener('load', () => resolve()); existing.addEventListener('error', () => reject()); return; }
    const s = document.createElement('script');
    s.src = PHYLLO_SDK_URL; s.async = true;
    s.onload = () => resolve();
    s.onerror = () => reject();
    document.head.appendChild(s);
  }).catch(() => {});
  return w.PhylloConnect ?? null;
}
...
function Setup() {
  ...
  const [phyllo, setPhyllo] = useState<PhylloState | null>(null);
  const [phylloBusy, setPhylloBusy] = useState(false);
  const [phylloMsg, setPhylloMsg] = useState<{ ok: boolean; text: string } | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/creator/data-export${qs}`)
      ...
      .catch(() => {});
    // Aggregator connector (Phyllo/InsightIQ) — only surfaces when env-configured.
    fetch(`/api/creator/connect/phyllo${qs}`)
      .then((r) => r.json())
      .then((d: PhylloState) => setPhyllo(d))
      .catch(() => setPhyllo(null));
  }, []);
  ...
  // Aggregator connect: mint an SDK token server-side, load Phyllo's Connect Web
  // SDK on demand, and open it. On a successful account link the webhook folds
  // real reach into the profile; we refresh the checklist when the widget closes.
  const connectPhyllo = async () => {
    if (phylloBusy) return;
    setPhylloBusy(true);
    setPhylloMsg(null);
    const qs = handle ? `?handle=${encodeURIComponent(handle.replace(/^@/, ''))}` : '';
    try {
      const r = await fetch(`/api/creator/connect/phyllo${qs}`, { method: 'POST' }).then((res) => res.json());
      if (!r?.ok || !r.sdk_token) {
        setPhylloMsg({ ok: false, text: 'Connect isn’t available right now. Please try again later.' });
        return;
      }
      const Connect = await loadPhylloSDK();
      if (!Connect) {
        setPhylloMsg({ ok: false, text: 'Couldn’t load the connect widget. Check your connection and retry.' });
        return;
      }
      const refresh = async () => {
        const [s, p] = await Promise.all([
          fetch(`/api/creator/setup${qs}`).then((res) => res.json()).catch(() => null),
          fetch(`/api/creator/connect/phyllo${qs}`).then((res) => res.json()).catch(() => null),
        ]);
        if (s) setData(s);
        if (p) setPhyllo(p);
      };
      const conn = Connect.initialize({
        clientDisplayName: 'Influencer Intel',
        environment: r.env,
        userId: r.user_id,
        token: r.sdk_token,
        workPlatformId: r.work_platform_id,
      });
      conn.on('accountConnected', () => { setPhylloMsg({ ok: true, text: 'Instagram connected — syncing your verified metrics…' }); void refresh(); });
      conn.on('exit', () => { void refresh(); });
      conn.open();
    } catch {
      setPhylloMsg({ ok: false, text: 'Something went wrong. Please try again.' });
    } finally {
      setPhylloBusy(false);
    }
  };
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-6 py-8">
        ...
        {loading ? (
          ...
          <>
            ...
            {/* Connect via aggregator (Phyllo/InsightIQ) — only when configured */}
            {phyllo?.available && (
              <div className="mt-5 rounded-2xl bg-white border border-border shadow-card p-5">
                <div className="flex items-start gap-3">
                  <span className="grid place-items-center w-9 h-9 shrink-0 rounded-lg text-white" style={{ background: `linear-gradient(135deg, ${ACCENT}, #9b7bff)` }}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 12l2 2 4-4" /><path d="M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0z" /></svg>
                  </span>
                  <div className="min-w-0 flex-1">
                    <div className="text-[14px] font-semibold text-ink-900">Connect Instagram for verified metrics</div>
                    <p className="mt-1 text-[12.5px] text-ink-500 leading-relaxed">Link securely through our data partner — real reach, saves and audience demographics, straight from Instagram. Read-only, disconnect anytime.</p>
                  </div>
                </div>

                {phyllo.connector?.status === 'connected' ? (
                  <div className="mt-3.5 flex items-center gap-2 text-[12.5px] font-medium" style={{ color: '#16a34a' }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>
                    Connected{phyllo.connector.username ? ` as @${phyllo.connector.username}` : ''} — your verified metrics are live.
                  </div>
                ) : (
                  <button onClick={connectPhyllo} disabled={phylloBusy}
                    className="mt-3.5 inline-flex items-center gap-2 px-4 py-2 text-[13px] font-semibold text-white rounded-xl transition-all duration-200 hover:brightness-105 hover:-translate-y-0.5 disabled:opacity-60 disabled:hover:translate-y-0"
                    style={{ background: `linear-gradient(135deg, ${ACCENT}, #9b7bff)` }}>
                    {phylloBusy && <span className="w-3.5 h-3.5 rounded-full border-2 border-white/40 border-t-white animate-spin" />}
                    {phylloBusy ? 'Opening…' : 'Connect Instagram'}
                  </button>
                )}
                {phylloMsg && (
                  <p className="mt-3 text-[12.5px] font-medium" style={{ color: phylloMsg.ok ? '#16a34a' : '#dc2626' }}>{phylloMsg.text}</p>
                )}
              </div>
            )}
