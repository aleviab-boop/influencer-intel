      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-6 py-8">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <Link
            ...
            className="inline-flex items-center gap-1.5 shrink-0 px-3.5 py-2 rounded-xl border border-border bg-white text-[13px] font-semibold text-ink-700 hover:border-[#d9d4f5] transition-colors duration-200"
          >
...
function SummaryTile({ label, value, sub, accent, href }: { label: string; value: string; sub: string; accent?: boolean; href?: string }) {
  ...
  return href
    ? <Link href={href} className={`${cls} group block transition-all duration-300 ease-out hover:-translate-y-1 hover:border-[#e3def9] hover:shadow-[0_16px_44px_rgba(108,77,246,0.16)]`}>{inner}</Link>
...
function DealCard({ d, handle }: { d: DealView; handle: string | null }) {
  ...
  return (
    <Link href={href} className="group block rounded-2xl bg-white border border-border shadow-card p-5 transition-all duration-300 ease-out hover:-translate-y-1 hover:border-[#e3def9] hover:shadow-[0_16px_44px_rgba(108,77,246,0.16)]">
