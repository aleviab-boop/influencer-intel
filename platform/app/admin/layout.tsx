...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-[#fafafc] text-[#111] font-sans">
      {/* sidebar */}
      <aside className="w-60 shrink-0 border-r border-[#ececf3] flex flex-col sticky top-0 h-screen" style={{ background: 'linear-gradient(180deg, #ffffff 0%, #fbfaff 55%, #f6f3ff 100%)' }}>
        <Link href="/lander" className="relative flex items-center gap-2 px-5 h-16 border-b border-[#f1f1f6]">
          <BrandMark size={28} />
      ...
      {/* main */}
      <main className="flex-1 min-w-0" style={{ background: 'radial-gradient(1100px 640px at 88% -8%, #f1ecff, #fafafc 58%), radial-gradient(760px 460px at 0% 100%, #eef4ff, transparent 60%)' }}>{children}</main>
    </div>
  );
}
