...
interface BrandCreator { id: string; handle: string; display_name: string | null; follower_count: number | string | null; primary_category: string | null; verified: boolean }
              ...
              {creators.length === 0 ? (
                ...
                <div className="rounded-2xl bg-white border border-border shadow-card overflow-hidden">
                  {creators.slice(0, 10).map((c) => (
                    <div key={c.id} className="flex items-center gap-3 px-4 py-2.5 border-b border-border-soft last:border-0 text-[13px]">
                      <div className="min-w-0 flex-1">
                        <div className="font-medium text-ink-900 truncate flex items-center gap-1.5">
                          {c.display_name || `@${c.handle}`}
                          {c.verified && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700">VERIFIED</span>}
                        </div>
                        <div className="text-[11px] text-ink-400 truncate">@{c.handle}{c.primary_category ? ` · ${c.primary_category}` : ''}</div>
                      </div>
